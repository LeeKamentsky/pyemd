"""_pyemd.pyx - Python wrapper around the FastEMD library
...
cdef extern from "pyemd_hat.hpp" nogil:
    cdef:
...
PYEMD_NO_FLOW = NO_FLOW
'''Calculate flow but ignore all bins with edges larger than max(C)'''
PYEMD_WITHOUT_TRANSHIPMENT_FLOW = WITHOUT_TRANSHIPMENT_FLOW
'''Calculate all flow except for the extra mass bin'''
PYEMD_WITHOUT_EXTRA_MASS_FLOW = WITHOUT_EXTRA_MASS_FLOW
   
def emd_hat_int32(p, q, c, 
                  extra_mass_penalty=None,
                  flow_type=PYEMD_NO_FLOW, gd_metric=False):
    ...
    :param flow_type: one of the following
        ...
        PYEMD_NO_FLOW don't calculate flow
        
        PYEMD_WITHOUT_TRANSHIPMENT_FLOW fills f with flows between
    ...
    :returns: The minimum cost of moving the dirt from p to q and, if
              the flow type is PYEMD_NO_FLOW, a len(p) by len(q) matrix
    ...
    pvf = &vf
    if flow_type == PYEMD_WITHOUT_TRANSHIPMENT_FLOW:
        if gd_metric:
